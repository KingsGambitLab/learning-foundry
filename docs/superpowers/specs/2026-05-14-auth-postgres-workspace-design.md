...
The migration must be done without disturbing the existing server running at `127.0.0.1:8010`. The new server runs in parallel on port `8020` so behavior can be diffed end-to-end before any cutover.
...
- Side-by-side verification script comparing 8010 (SQLite) and 8020 (Postgres) JSON outputs.
...
Compares 8010 (SQLite, untouched) to 8020 (Postgres, freshly migrated) end-to-end.

1. Hit `127.0.0.1:8010` and `127.0.0.1:8020` for each of these endpoints, capture JSON:
   - `GET /v1/lms/courses` (catalog)
   - `GET /v1/lms/enrollments?learner_id=local-learner` (against 8010) vs `GET /v1/lms/enrollments` (against 8020 with the seed-learner's session cookie). The two endpoints address the same logical learner under different keys; the diff normalizes `learner_id` before comparing so the rewrite from `"local-learner"` to `seed_learner_id` is not flagged as a mismatch.
...
4. Spot-check: log in as the seed learner against 8020, launch the workspace, list workspace files — assert the file list matches what 8010 returns for the same enrollment.
...
- nginx (or Caddy) terminates TLS via Let's Encrypt and reverse-proxies to the FastAPI app on `:8020`. Dynamic code-server host ports are reverse-proxied through the same origin via subpaths or per-session subdomains (decision deferred — current local dev exposes raw host ports).
...
| What | How |
| --- | --- |
| Storage swap parity | Side-by-side JSON diff of 8010 (SQLite) vs 8020 (Postgres) for the catalog, enrollments, workflow, timeline, and experience endpoints. Exit non-zero on any unaccounted mismatch. |
...
| End-to-end on 8020 | Manually: register a learner against 8020, enroll in a course copied over by the migrator, launch the workspace, write a file, submit, see the grade report. |
...
4. **M4 — Workspace path change.** Update `_workspace_root`, run the workspace-rename migrator phase, verify launch + file read/write + submit on 8020.
