...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "Rubric kwarg signatures — emit EXACTLY these field names "
        "(observed-in-the-wild kwarg drift was the #1 reason the live "
        "RAG smoke produced 0/18 grading verdicts — every scenario's "
        "rubrics crashed with ``unexpected keyword argument``):\n"
        "  - ``schema_match``: ``target`` (path str), ``must_have_fields`` "
        "(list[str]). DO NOT emit ``schema`` (JSON Schema dict) — list "
        "the required field names directly.\n"
        "  - ``literal_match``: ``target``, ``expected``. NOT ``value``.\n"
        "  - ``regex_match``: ``target``, ``pattern``.\n"
        "  - ``numeric_range``: ``target``, ``min_value``, ``max_value``. "
        "NOT ``min``/``max``.\n"
        "  - ``subset_match``: ``target``, ``acceptable_source`` (path "
        "into captures or ``setup_data.*``), ``acceptable_key`` "
        "(optional), ``min_overlap`` (float, default 1.0).\n"
        "  - ``behavioral_equivalence``: ``target``, ``expected`` "
        "(literal value, NOT a path), ``case_sensitive`` (bool).\n"
        "  - ``oracle_set_overlap``: ``target``, ``gold_set_path`` (path "
        "into ``setup_data`` — NO ``setup_data.`` prefix needed), "
        "``target_key`` (optional), ``min_recall`` (float, default "
        "0.5), ``top_k`` (optional int).\n"
        "  - ``llm_judge_coverage``: ``target`` (path to answer), "
        "``must_contain_facts`` (list[str], the facts the answer must "
        "include), ``judge_context_path`` (optional path), "
        "``strictness`` (\"lenient\"|\"strict\").\n"
        "  - ``llm_judge_semantic_eq``: ``target`` (path to learner "
        "answer), ``gold_path`` (path into the MERGED ctx — use the "
        "``setup_data.<file_stem>.<key>`` prefix), ``alt_path`` "
        "(optional), ``strictness``.\n"
        "  - ``llm_judge_false_premise``: ``target`` (path to learner "
        "answer), ``expected_falsity_path`` (optional path with "
        "``setup_data.`` prefix).\n\n"
        "Path convention: ``oracle_set_overlap.gold_set_path`` walks "
        "``ctx.setup_data`` DIRECTLY (path is RELATIVE to setup_data, no "
        "``setup_data.`` prefix). ``llm_judge_*`` paths use a MERGED "
        "context — these REQUIRE the ``setup_data.`` prefix. Don't mix "
        "the two.\n\n"
