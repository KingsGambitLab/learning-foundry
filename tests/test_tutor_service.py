...
def _make_fake_store(session=None, submissions=None) -> MagicMock:
    store = MagicMock()
    # WorkflowStore Protocol (Postgres) resolves a session by listing all
    # sessions and filtering by id — there is no get-by-session-id method.
    store.list_all_learner_workspace_sessions.return_value = [session] if session else []
...
class TutorServiceTest(unittest.TestCase):
    ...
    def test_chat_with_store_but_no_matching_session_uses_basic_prompt(self) -> None:
        ...
        store.list_all_learner_workspace_sessions.assert_called_once_with()
        call_kwargs = fake_client.with_options.return_value.messages.create.call_args[1]
        system_text = call_kwargs["system"][0]["text"]
        user_content = call_kwargs["messages"][0]["content"]
