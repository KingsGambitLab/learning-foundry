...
class LearnerStudioServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.existing_session = LearnerWorkspaceSession(
            ...
            module_id="exercise/01-contract",
    ...
    def test_launch_editor_reuses_running_session_when_workspace_matches_current_module(self) -> None:
        ...
        with (
            ...
            patch.object(service, "_container_current_module_id", return_value="exercise/01-contract"),
        ...
        self.assertEqual(refreshed.module_id, "exercise/01-contract")
        self.assertEqual(refreshed.status, LearnerWorkspaceSessionStatus.running)

    def test_launch_editor_restarts_running_session_when_workspace_marker_is_missing(self) -> None:
        service = LearnerStudioService(image_name="course-gen-learner-studio:test")

        with (
            patch.object(service, "_ensure_image"),
            patch.object(service, "_container_running", return_value=True),
            patch.object(service, "_container_current_module_id", return_value=None),
